...
abstract class AbstractApi {
    ...
    protected function getBaseUrlWithRegion() {
        return sprintf(self::$defaultDomain, $this->lolApi->getRegion(), $this->lolApi->getRegion())
                . $this->version . '/';
    }
    ...
    protected function makeRequest($path, array $params = null) {
        $queryParams = $params != null ? array_merge(['api_key' => $this->lolApi->getApiKey()], $params) : ['api_key' => $this->lolApi->getApiKey()];

        $url = $this->buildUrl($path, $queryParams);

        if ($this->lolApi->getCache()->has($url)) {
            return $this->buildResponse($url, $this->lolApi->getCache()->get($url), true);
        }
        try {
            $response = $this->lolApi->getClient()->get($url);

            return $this->buildResponse($url, json_decode($response->getBody(), true), false);
    ...
    private function handleException(ClientException $ex) {
        ...
        if ($code >= 500) {
            throw new HttpServerException('Service unavailable');
        }
        switch ($code) {
            case 400:
                throw new Http400Exception('Bad request');
            case 401:
                throw new Http401Exception('Invalid API key');
            case 403:
                throw new Http403Exception('Invalid API key');
            case 404:
            ...
            default :
                throw new Http400Exception($code);
        }
    }
